 ...
 * @author Vasile Dirla
 */
public class AcquireJobsDueRunnable implements Runnable {

  private static Logger log = LoggerFactory.getLogger(AcquireJobsDueRunnable.class);
  ...
  public AcquireJobsDueRunnable(AsyncExecutor asyncExecutor) {
  ...
  public synchronized void run() {
    ...
    while (!isInterrupted) {
      ...
      try {
        AcquiredExecutableJobEntities acquiredJobs = commandExecutor.execute(new AcquireExecutableJobsDueCmd(asyncExecutor));

        boolean allJobsSuccessfullyOffered = true;
        for (LockedJobEntity job : acquiredJobs.getJobs()) {
